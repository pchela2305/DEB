...
namespace DotnetMakeDeb.Deb
{
	...
	internal class DebPackage
	{
		...
		public DebControlParams controlParams;
		...
		public void ReadSpecificationDeb(string path)
		{
			srcBasePath = Path.GetDirectoryName(path);

			controlParams = new DebControlParams();


			var confDir = Path.Combine(path, "DEBIAN");

            #region get control file check
            var controlFile = File.ReadAllText(Path.Combine(confDir, "control"));

			var m = Regex.Match(controlFile, @"package\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Package = ResolveVariables(m.Groups[1].Value.Trim());
			
			m = Regex.Match(controlFile, @"version\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Version = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"convert-semver\s*", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.ConvertFromSemVer = true;

			m = Regex.Match(controlFile, @"section\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Section = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"prio(?:rity)?\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Priority = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"arch(?:itecture)?\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Architecture = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"depends\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
			{
				string depends = ResolveVariables(m.Groups[1].Value.Trim());
				if (string.IsNullOrWhiteSpace(controlParams.Depends))
			...
			m = Regex.Match(controlFile, @"pre-depends\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
			{
				string preDepends = ResolveVariables(m.Groups[1].Value.Trim());
				if (string.IsNullOrWhiteSpace(controlParams.PreDepends))
			...
			m = Regex.Match(controlFile, @"conflicts\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Conflicts = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"maintainer\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Maintainer = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"homepage\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Homepage = ResolveVariables(m.Groups[1].Value.Trim());

			m = Regex.Match(controlFile, @"desc(?:ription)?\s*:\s*(.+)", RegexOptions.IgnoreCase);
			if (m.Success)
				controlParams.Description = ResolveVariables(m.Groups[1].Value.Trim());
			#endregion
			...
			// preinst
			if (File.Exists(Path.Combine(confDir, "preinst")))
				preInstFileName = Path.Combine(confDir, "preinst");

			// postinst
			if (File.Exists(Path.Combine(confDir, "postinst")))
				postInstFileName = Path.Combine(confDir, "postinst");

			// prerm
			if (File.Exists(Path.Combine(confDir, "prerm")))
				preRmFileName = Path.Combine(confDir, "prerm");

			// postrm
			if (File.Exists(Path.Combine(confDir, "postrm")))
				postRmFileName = Path.Combine(confDir, "postrm");
			...
			{
				var destPath = file.Replace(path, "").TrimStart('\\');

				if (destPath.StartsWith("DEBIAN"))
				{
					continue;
				}
				...
				fileItem.DestPath  = destPath.Replace(@"\","/");
				//fileItem.Mode    = Convert.ToInt32(ResolveVariables(m.Groups[3].Value), 8);
				//fileItem.UserId  = Convert.ToInt32(ResolveVariables(m.Groups[4].Value));
				//fileItem.GroupId = Convert.ToInt32(ResolveVariables(m.Groups[5].Value));
				foreach (var fi in ResolveFileItems(fileItem))
			...
			#region add empty dir
			foreach(var dir in Directory.GetDirectories(path, "*", SearchOption.AllDirectories))
			{
				var destPath = dir.Replace(path, "").TrimStart('\\');
